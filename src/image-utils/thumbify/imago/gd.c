...
gdImagePtr gdImageCreate(int sx, int sy)
{
	...
	/* this bzero added by jon to stop purify from bitching about
	   uninitialized memory reads later when GIFEncode writes out 
	   the color table
	*/
	bzero(im, sizeof(gdImage));
...
void gdImageGif(gdImagePtr im, FILE *out)
{
	...
	fprintf(stderr, "gdImageGif: total colors: %d, BitsPerPixel: %d\n",
		im->colorsTotal, BitsPerPixel);
...
/*
  reads off image data or returns 0
*/
gdImagePtr
gdImageHeaderFromGif(FILE *fd)
{
       int total_colors = 0;
       int imageNumber;
       int BitPixel;
       ...
       for (;;) {
               if (! ReadOK(fd,&c,1)) {
                       /* fprintf(stderr, "gdImageHeaderFromGif: ReadOK failed to read 1 byte\n");
                       fprintf(stderr, "gdImageHeaderFromGif: incomplete Gif image data\n");
		       return 0; */
		       im->colorsTotal = total_colors;
                       return im;
               }
               if (c == ';') {         /* GIF terminator */
                       int i;
                       if (imageCount < imageNumber) {
			       fprintf(stderr, "gdImageHeaderFromGif: imageCount < imageNumber\n");
                               return 0;
                       }
                       /* Terminator before any image was declared! */
                       if (!im) {
			      fprintf(stderr, "gdImageHeaderFromGif: Terminator before any image was declared!\n");
                              return 0;
                       }
               ...
               if (c == '!') {         /* Extension */
                       if (! ReadOK(fd,&c,1)) {
                               fprintf(stderr, "gdImageHeaderFromGif: ReadOK failed after ! Extension\n");
                               return 0;
                       }
               ...
               if (! ReadOK(fd,buf,9)) {
                       fprintf(stderr, "gdImageHeaderFromGif: ReadOK failed reading 9 chars\n");
	               return 0;
               }
		 ...
		 if (!(im = gdImageCreate(imw, imh))) {
		   fprintf(stderr, "gdImageHeaderFromGif: gdImageCreate failed\n");
		   return 0;
		 }
	       }
               ...
               if (! useGlobalColormap) {
                       ...
                       if (ReadColorMap(fd, bitPixel, localColorMap)) { 
                                 fprintf(stderr, "gdImageHeaderFromGif: ReadColorMap failed\n");
                                 return 0;
                       }
		       ReadImage2(fd);
               } else {
...
gdImageCreateFromGif(FILE *fd)
{
       ...
       for (;;) {
               if (! ReadOK(fd,&c,1)) {
                       fprintf(stderr, "gdImageCreateFromGif: ReadOK failed to read 1 byte\n");
                       fprintf(stderr, "gdImageCreateFromGif: incomplete Gif image data\n");
		       /* return 0; */
                       return im;
               }
               if (c == ';') {         /* GIF terminator */
                       int i;
		       /*fprintf(stderr, "gdImageCreateFromGif: GIF terminator\n");*/
                       if (imageCount < imageNumber) {
			       fprintf(stderr, "gdImageCreateFromGif: imageCount < imageNumber\n");
                               return 0;
                       }
                       /* Terminator before any image was declared! */
                       if (!im) {
			      fprintf(stderr, "gdImageCreateFromGif: Terminator before any image was declared!\n");
                              return 0;
                       }
		       /*fprintf(stderr, "gdImageCreateFromGif: im->colorsTotal: %d\n", im->colorsTotal);*/
		       /* Check for open colors at the end, so
                          we can reduce colorsTotal and ultimately
                          BitsPerPixel */
                       for (i=((im->colorsTotal-1)); (i>=0); i--) {
               ...
               if (c == '!') {         /* Extension */
                       if (! ReadOK(fd,&c,1)) {
                               fprintf(stderr, "gdImageCreateFromGif: ReadOK failed after ! Extension\n");
                               return 0;
                       }
                       DoExtension(fd, c, &Transparent);
                       continue;
               ...
               ++imageCount;
	       if(imageCount != imageNumber)
		 fprintf(stderr, "gdImageCreateFromGif: imageCount: %d, imageNumber :%d\n",
			 imageCount, imageNumber);
               if (! ReadOK(fd,buf,9)) {
                       fprintf(stderr, "gdImageCreateFromGif: ReadOK failed reading 9 chars\n");
	               return 0;
               }
	       ...
	       if(!im){
		 if (!(im = gdImageCreate(imw, imh))) {
		   fprintf(stderr, "gdImageCreateFromGif: gdImageCreate failed\n");
		   return 0;
		 }
	       }
               im->interlace = BitSet(buf[8], INTERLACE);
               if (! useGlobalColormap) {
                       if (ReadColorMap(fd, bitPixel, localColorMap)) { 
                                 fprintf(stderr, "gdImageCreateFromGif: ReadColorMap failed\n");
                                 return 0;
                       }
...
LWZReadByte(FILE *fd, int flag, int input_code_size)
{
       ...
       int max_lwz_bits_size = (1<<(MAX_LWZ_BITS))*2;
       ...
       while ((code = GetCode(fd, code_size, FALSE)) >= 0) {
		 ...
		 /*
		 fprintf(stderr, "code: %d, clear_code: %d\n",
			 code, clear_code);
		 fprintf(stderr, "size: %d\n", sp - stack);
 		 */
		 if(sp > stack + max_lwz_bits_size){
		   fprintf(stderr, "sp too large\n");
		   /*return *--sp;*/
		   return -1;
		 }
		 if(sp < stack){
		   fprintf(stderr, "sp too small\n");
		   return -1;
		 }
		 if(code > (1 << MAX_LWZ_BITS) - 1){
...
ReadImage2 (FILE *fd)
{
  ...
  while(fread(buffer, sizeof(char), 4096, fd) > 0);
}
...
ReadImage(gdImagePtr im, FILE *fd, int len, int height, unsigned char (*cmap)[256], int interlace, int ignore)
{
	 ...
	 im->open[i] = 1;
       ...
       */
       if (ignore) {
	 fprintf(stderr, "trying to ignore picture\n");
...
I've added another unsigned char to the gd header to indicate the
type: 1 is indexed color, 2 is greyscale, 3 is rgb.

indexed color format:
1 unsigned char type
1 word width
1 word height
1 unsigned char total colors
1 word transparent color index
256 * 3 unsigned chars colormap
width * height unsigned chars data
...
1 word width
1 word height
width * height * 3 unsigned chars data

*/

void gdImageGd2 (gdImagePtr im, FILE *out)
{
  int x, y;
  int i;
  int trans;
  putc((unsigned char)1, out); /* indexed color */
  gdPutWord(im->sx, out);
  gdPutWord(im->sy, out);
  putc((unsigned char)im->colorsTotal, out);
  trans = im->transparent;
