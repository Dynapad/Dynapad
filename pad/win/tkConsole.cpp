...
static int
ConsoleOutput(ClientData instanceData, char *buf, int toWrite, int *errorCode)
    /* instanceData;		 Indicates which device to use. */
    /* buf;				 The data buffer. */
    /* toWrite;			 How many bytes to write? */
 ...
 */
	...
	/* ARGSUSED */
static int
ConsoleInput(ClientData instanceData, char *buf, int bufSize, int *errorCode)
    /* instanceData;		Unused. */
    /* buf;				Where to store data read. */
    /* bufSize;			How much space is available
                                         * in the buffer? */
    /* errorCode;			Where to store error code. */
{
 ...
 */
	...
	/* ARGSUSED */
static int
ConsoleClose(ClientData instanceData, Tcl_Interp *interp)
    /* instanceData;	Unused. */
    /* interp;		Unused. */
{
    return 0;
 ...
 */
	...
	/* ARGSUSED */
static void
ConsoleWatch(ClientData instanceData, int mask)
    /* instanceData;		 	Device ID for the channel. */
    /* mask;				OR-ed combination of
 ...
 */
	...
	/* ARGSUSED */
static int
ConsoleReady(ClientData instanceData, int mask)
    /* instanceData;	 	Device ID for the channel. */
    /*mask;				OR-ed combination of
                                         * TCL_READABLE, TCL_WRITABLE and
                                         * TCL_EXCEPTION, for the events
 ...
 */
	...
	/* ARGSUSED */
static Tcl_File
ConsoleFile(ClientData instanceData, int direction)
    /* instanceData;		Device ID for the channel. */
...
static int
ConsoleCmd(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
    /* clientData;		Not used. */
...
static int
InterpreterCmd(ClientData clientData, Tcl_Interp *interp, int argc, char **argv)
    /* clientData;		Not used. */
    /* interp;			Current interpreter. */
    /* argc;			Number of arguments. */
    /* argv;			Argument strings. */
...
void 
ConsoleDeleteProc(ClientData clientData) 
{
    ConsoleInfo *info = (ConsoleInfo *) clientData;
...
void
TkConsolePrint(Tcl_Interp *interp, int devId, char *buffer, long size)
    /* interp;		Main interpreter. */
    /* devId;		TCL_STDOUT for stdout, TCL_STDERR for stderr */
    /* buffer;		Text buffer. */
