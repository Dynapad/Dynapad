...
int main(int argc, char **argv)
{
    ...
    match = XftFontMatch(display, screen, pat, &result);
    if (!match) {
      printf("XFtFontMatch failed: %s\n",
        print_xftresult(result));
    }
    result = XftPatternGetString(match, "file", 0, &s);
    if (result != XftResultMatch) {
      printf("XFtPatternGetString match failed: %s\n",
        print_xftresult(result));
      exit(EXIT_FAILURE);
    }
    printf("match %s\n", s);
